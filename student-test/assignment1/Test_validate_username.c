...
void test_validate_my_username()
{
    ...
    const char *get_my_username = my_username();
    const char *username_from_conf_file = malloc_username_from_conf_file();

    // printf("%s", get_my_username);
    // printf("%s", username_from_conf_file);

    // TEST_ASSERT_TRUE_MESSAGE(get_my_username, username_from_conf_file);
    TEST_ASSERT_EQUAL_STRING_MESSAGE(get_my_username, username_from_conf_file, "right ? - right !");
}
